...
app.post('/Form',(req,res)=>{
    const sql = `Insert into S20190010007 (pname,rollno,semester,pdetails,gpca,techused) Values (?,?,?,?,?,?)`
    const book = [req.body.pname, req.body.rollno, req.body.sem, req.body.pdetails,req.body.cgpa, req.body.techused]
    db.run(sql,book,err=>{
        if(err){
            return console.log(err.message)
        }
...
app.get('/SortDetails',(req,res)=>{
    const sql = "Select * from S20190010007 ORDER BY cgpa ASC"
    db.all(sql,(err,rows)=>{
        if(err){
            return console.log(error)
        }
